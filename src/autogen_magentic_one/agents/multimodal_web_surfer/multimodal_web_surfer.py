...
class MultimodalWebSurfer(BaseWorker):
    ...
    async def _execute_tool(
        ...
        # **Modification Start: Heuristic for Recognized Regulatory Sites**
        if name == "visit_url":
            url = args.get("url")
            # Simple heuristic: If url contains certain keywords, trust and go directly
            if any(k in url.lower() for k in ["fda.gov", "ema.europa.eu", "tga.gov.au", "canada.ca"]):
                # Directly navigate
                await self._visit_page(url)
                action_description = f"I navigated directly to recognized regulatory site: {url}."
                return False, "Navigated directly to recognized regulatory site. TERMINATE"
        # **Modification End**
    ...
    async def __generate_reply(self, cancellation_token: CancellationToken) -> Tuple[bool, UserContent]:
        ...
        # **Modification Start: Enhanced Prompt for Recognized Regulatory Sites**
        # Include a prompt section that leverages the heuristic navigation
        # This can be expanded to include more domain-specific instructions as needed
        text_prompt = f"""
